...
\begin{figure}\label{fig:weakGPH}
    ...
    \caption{Weak GPH online regime: population loss ratio between SGD and GD trajectories across $N$ SGD trajectories ($\gamma=1.5$, noise $=0.0$). \textbf{What we want instead: take one seed plot across regimes and one width (e.g. 50 or 100). First row: online regime across initializations, second row: ratio loss SGD/GD (online regime). Third and fourth rows: offline regime across initializations and ratio loss SGD/GD offline regime. Ignore label noise. Include other figures with label noise, other seeds and widths in the appendix (with a similar format).}}
...
\begin{figure}\label{fig:drift-diffusion-online}
    ...
    \caption{Drift and diffusion: population loss ratio between SGD and GD trajectories across $N$ SGD trajectories ($\gamma=1.5$, noise $=0.0$, batch size $=1$). \textbf{What we want instead: take one seed, one width (e.g. 50 or 100) and online regime. First row: plot test loss; second row: drift; third row: diffusion; fourth row: ratio drift diffusion; each column is a different initialization (saddle to saddle vs mean field vs NTK), no label noise. Include other figures with label noise, other seeds and widths, offline in the appendix (with a similar format and since offline is similar).}}
    \label{fig:gph_online}
\end{figure}
...
\section*{GPH function and parameter spaces}
A stronger version of GPH is that SGD and GD selects functions that are close in function space (function space GPH). A further strengthening of the GPH is the hypothesis that SGD and GD (or SGD with large batch size) select parameters that are close in parameter space. We investigate both of these hypotheses in DLNs by monitoring the distances in function space and parameter space between the functions selected by GD and SGD across many runs of SGD.

\begin{figure}\label{fig:drift-diffusion-online}
    \centering
    \includegraphics[width=\linewidth]{figures/Distances G1.0 Noise0.0 W100.png}
    \caption{Distance in function and parameter space: population loss ratio between SGD and GD trajectories across $N$ SGD trajectories ($\gamma=1.0$, noise $=0.0$, batch size $=1$). \textbf{TODO: take one seed, one width (e.g. 50 or 100) and online regime. same rows and each column is a different initialization (saddle to saddle vs mean field vs NTK), no label noise. Include other figures with label noise, other seeds and widths, offline in the appendix (with a similar format and since offline is similar).}}
    \label{fig:gph_online}
\end{figure}
\section*{Rank patterns}
Rank patterns are connected to the complexity of the solutions selected by the learning dynamics. Indeed from the work of \cite{simon} the rank patterns parametrize the orbits of the global minima of the loss landscape under the action of the group $GL_d$ and their codimension is proportional to the local learning coefficient, which is a measure of model complexity \cite{llc}. We test that GD and SGD selects solution with the same singular values of all the partial products of the DLN. We find that the dynamics of the singular values of the partial products of DLNs are similar for GD and SGD across the different regimes of learning dynamics. This suggests that GD and SGD select functions of the same complexity in deep linear networks.
\begin{figure}\label{fig:drift-diffusion-online}
    \centering
    \includegraphics[width=\linewidth]{figures/PP 2.2.png}
    \caption{Distance in function and parameter space: population loss ratio between SGD and GD trajectories across $N$ SGD trajectories ($\gamma=1.0$, noise $=0.0$, batch size $=1$). \textbf{TODO: take one seed, one width (e.g. 50 or 100) and online regime each column is a different initialization (saddle to saddle vs mean field vs NTK); each row is a different partial product no label noise. Include other figures with label noise, other seeds and widths, offline in the appendix (with a similar format and since offline is similar).}}
    \label{fig:gph_online}
\end{figure}
\section*{Appendix}
